...
public class ItemInteractionViewModel : INotifyPropertyChanged
{
    ...
    private async void OnEquipHand()
    {
        ...
        try
        {
            ...
            if (slotsNeeded > AvailableHandSlots)
            {
                await Application.Current.MainPage.DisplayAlertAsync(
                    "Not Enough Slots",
                    $"This item requires {slotsNeeded} hand slot(s). You have {AvailableHandSlots} available.",
                    "OK");
                return;
            }
            ...
            {
                var existing = _charDataService.GetEquippedHandSlot(i);
                if (existing == null)
                {
                    if (targetSlot == -1) targetSlot = i;
                    consecutiveFreeSlots++;
                    if (consecutiveFreeSlots >= slotsNeeded)
                        break;
                }
                else
                {
                    // Reset if we hit an occupied slot
                    targetSlot = -1;
                    consecutiveFreeSlots = 0;
                }
            }

            if (targetSlot == -1 || consecutiveFreeSlots < slotsNeeded)
            {
                await Application.Current.MainPage.DisplayAlertAsync(
                    "No Slots Available",
                    $"Could not find {slotsNeeded} consecutive free hand slots.",
                    "OK");
                return;
            }
